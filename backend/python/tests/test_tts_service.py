...
from ivr_assessor.tts_service import (
    OpenAITTS,
    PiperNotFoundError,
    PiperTTS,
    _LRUCache,
...
def test_piper_raises_when_binary_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("ivr_assessor.tts_service.shutil.which", return_value=None):
        tts = PiperTTS()
...
def test_piper_raises_when_voice_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("ivr_assessor.tts_service.shutil.which", return_value="/usr/bin/piper"):
        tts = PiperTTS()
