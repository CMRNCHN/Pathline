...
def _build_summary(
    ...
    if bundle.replay_state is not None:
        ...
        node_count = int((bundle.runtime_diagnostics or {}).get("graph_node_count") or 0)
...
def _build_session_metadata(bundle: ResolvedReplayBundle) -> SessionMetadataSection:
    ...
    diag = bundle.runtime_diagnostics or {}
...
def _build_media_status(bundle: ResolvedReplayBundle) -> MediaStatusSection:
    ...
    diag = bundle.runtime_diagnostics or {}
    media = bundle.media_metadata or {}
...
def _build_identity(bundle: ResolvedReplayBundle) -> IdentitySection:
    ...
    diag = bundle.runtime_diagnostics or {}
    # If no event log was loaded, the session doesn't exist in storage
    event_log_available = any(
        e.artifact == "event_log" and e.available
        for e in bundle.artifact_availability.entries
    )
    source_kind = "event_log" if event_log_available else "empty"
...
class InspectionService:
    """Thin class wrapper around inspect_session for callers that expect an object interface."""

    def __init__(self, resolver: BundleResolver | None = None) -> None:
        self._resolver = resolver

    def inspect(self, session_id: str) -> ReplayInspectionReport:
        return inspect_session(session_id, resolver=self._resolver)


# build_inspection_report is the alias used by replay_routes.py
build_inspection_report = inspect_session

__all__ = ["inspect_session", "build_inspection_report", "InspectionService"]
