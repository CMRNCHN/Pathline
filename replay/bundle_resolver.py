...
class ResolvedReplayBundle:
    ...
    session_listing: dict[str, Any] = field(default_factory=dict)
...
class BundleResolver:
    ...
    def __init__(
        ...
        media_replay_service: Optional[MediaReplayService] = None,
        ...
        self.media_service = media_replay_service or MediaReplayService()
    ...
    def resolve(self, session_id: str) -> ResolvedReplayBundle:
        """Locate and load all available artifacts for a session."""
        ...
        # 2. Reconstruct runtime state → runtime_diagnostics dict
        runtime_diagnostics: dict[str, Any] = {}
        try:
            replay_state = self.replay_service.load_replay(session_id)
            state_available = replay_state is not None
            if replay_state is not None:
                runtime_diagnostics = {
                    "session_id": replay_state.session_id,
                    "call_sid": getattr(replay_state, "call_sid", None),
                    "call_status": getattr(replay_state, "call_status", None),
                    "graph_node_count": len(getattr(replay_state, "nodes", {}) or {}),
                    "transcript_count": len(getattr(replay_state, "transcripts", []) or []),
                    "visited_node_count": len(getattr(replay_state, "visited_nodes", []) or []),
                    "active_path_length": len(getattr(replay_state, "active_path", []) or []),
                    "metrics": getattr(replay_state, "metrics", {}) or {},
                    "recording_reference": getattr(replay_state, "recording_reference", None),
                    "waveform_reference": getattr(replay_state, "waveform_reference", None),
            ...
            entries.append(ArtifactAvailabilityEntry(
                ...
                detail="State reconstructed" if state_available else "Failed to reconstruct state",
            ))
            if not state_available:
                missing.append("runtime_diagnostics")
        except Exception as e:
            entries.append(ArtifactAvailabilityEntry(
                artifact="runtime_diagnostics",
                available=False,
        ...
        try:
            ...
            if raw_media_meta is None:
                entries.append(ArtifactAvailabilityEntry(
                    artifact="recording", available=False,
                    detail="Media service returned no metadata",
                ))
                missing.append("recording")
                entries.append(ArtifactAvailabilityEntry(
                    artifact="waveform", available=False,
                    detail="Media service returned no metadata",
                ))
            ...
            entries.append(ArtifactAvailabilityEntry(
                artifact="recording", available=False, detail=f"Error: {e}",
            ))
            missing.append("recording")
            entries.append(ArtifactAvailabilityEntry(
                artifact="waveform", available=False, detail=f"Error: {e}",
            ))
            missing.append("waveform")

        # 5. Resolve bookmarks & annotations (missing → added to missing list)
        try:
            ...
            entries.append(ArtifactAvailabilityEntry(
                artifact="bookmarks", available=False, detail=f"Error: {e}",
            ))
            missing.append("bookmarks")
        ...
        try:
            ...
            entries.append(ArtifactAvailabilityEntry(
                artifact="annotations", available=False, detail=f"Error: {e}",
            ))
            missing.append("annotations")

        # Build session listing from the event file path
        session_listing: dict[str, Any] = {}
        ...
        artifact_availability = ArtifactAvailabilitySection(entries=entries, missing=missing)
