...
from runtime.media.audio_pipeline import (
...
def _make_vad_with_mock(speech_sequence: list[bool], on_utterance=None) -> tuple[VoiceActivityDetector, Any]:
    ...
    with patch("runtime.media.audio_pipeline._webrtcvad", mock_module):
...
def test_vad_import_error_raises_clearly() -> None:
    ...
    with patch("runtime.media.audio_pipeline._webrtcvad", None):
