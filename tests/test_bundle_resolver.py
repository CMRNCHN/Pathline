...
class TestBundleResolver(unittest.TestCase):
    def setUp(self):
        ...
        self.resolver = BundleResolver(
            ...
            media_replay_service=self.media_service,
    ...
    def test_resolve_bundle_full_availability(self):
        ...
        bundle = self.resolver.resolve(session_id)

        self.assertIsInstance(bundle, ResolvedReplayBundle)
        self.assertEqual(bundle.session_id, session_id)
        self.assertEqual(bundle.raw_events, events)
        self.assertEqual(bundle.replay_state, state)
        ...
        # Verify availability
        availability = bundle.artifact_availability
        ...
        self.assertIn("event_log", available_artifacts)
        self.assertIn("runtime_diagnostics", available_artifacts)
    ...
    def test_resolve_bundle_partial_availability(self):
        ...
        self.assertIn("runtime_diagnostics", missing_artifacts)
        ...
        self.assertNotIn("event_log", missing_artifacts)
    ...
    def test_resolve_bundle_handles_service_exceptions(self):
        ...
        availability = bundle.artifact_availability
        self.assertIn("event_log", availability.missing)

        events_entry = next(e for e in availability.entries if e.artifact == "event_log")
