...
|-------|---------|--------|
| Native SIP + Whisper tests | `cd desktop/src-tauri && cargo test` | **21 passed, 0 failed**, including real bundled-model inference |
...
| Headless prep + STT fixture | `cd client && npm run stt:fixture` | **PASS** (waits, timeouts, structured detect, DTMF `w`, pipeline + selection) |
...
21 tests exercise the signaling/media paths and native model runtime, notably:
...
First checks the free prep slice (wait tokens, Step timeout reporting,
structured detect JSON, and DTMF `w` pauses), then drives synthetic PCM through
the **production path**
...
| 4 | DTMF ledger stores hash + digit count, never plaintext | **PASS** | `runSession.ts`: `DTMF_SENT` metadata stores `{ step, digits, hash }`, where `digits` excludes `w` pauses and `hash` is SHA-256. `sip_bridge.rs` logs `count` + `short_hash(&digits)` only, and `dtmf_sent` carries `count=`. Ledger types comment: "Never raw secrets." |
