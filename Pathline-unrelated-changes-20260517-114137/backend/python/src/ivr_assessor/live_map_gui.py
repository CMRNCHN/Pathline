...
def _ensure_stream_server() -> StreamingServer | None:
    ...
    if _persistent_stream is None:
        ...
        try:
            ...
            bound = _wait_for_port("127.0.0.1", _STREAM_PORT, timeout=8.0)
            if not bound:
...
def _diagnose() -> dict:
    ...
    stt_backend = os.environ.get("STT_BACKEND", "faster-whisper").lower()
    if stt_backend == "simulated":
        stt = {"ok": True, "backend": stt_backend, "mode": "simulated", "error": None}
    elif stt_backend == "deepgram":
        stt = {"ok": bool(deepgram_key), "backend": stt_backend, "mode": "cloud", "error": deepgram["error"]}
    else:
        ...
        stt = {
            ...
            "backend": stt_backend,
            "mode": "local",
            "error": None if has_faster_whisper else "Missing faster-whisper package",
        }
        if not has_faster_whisper:
            issues.append("faster-whisper package missing — install backend dependencies or use STT_BACKEND=simulated")
    ...
    if tts_backend == "openai":
        ...
        tts = {
            ...
            "backend": tts_backend,
            "mode": "cloud",
            "error": None if openai_ready else "Missing OPENAI_API_KEY",
        }
    else:
        piper_binary = os.environ.get("PIPER_BINARY") or shutil.which("piper")
        piper_voice = os.environ.get("PIPER_VOICE", "")
        voice_ready = bool(piper_voice and Path(piper_voice).expanduser().exists())
        tts = {
            "ok": bool(piper_binary and voice_ready),
            "backend": tts_backend,
            "mode": "local",
            "binary": piper_binary,
            "voice": piper_voice,
            "error": None if piper_binary and voice_ready else "Missing piper binary or PIPER_VOICE",
        }
    import socket
    ...
    tunnel = {
        ...
        "local_only": configured_backend == "none",
    }
    if configured_backend == "none":
        pass
    elif configured_backend == "cloudflare" or cloudflare_configured:
    ...
    return {
        ...
        "ok": not issues,
        "server_active": True,
        "tunnel_active": bool(ngrok_url or _default_stream_url or configured_backend == "none"),
        "twilio_ready": bool(twilio.get("ok")),
        "media_stream_ready": stream_listening,
        "stt_ready": bool(stt.get("ok")),
        "tts_ready": bool(tts.get("ok")),
        "ai_ready": stt_backend == "simulated" or bool(stt.get("ok")),
    }
...
def _runtime_metrics_payload() -> dict[str, Any]:
    ...
    startup_snapshot = STATE.startup_snapshot()
    startup_events = startup_snapshot.get("events", [])
    startup_state = "pending"
    if startup_events:
        latest_stage = startup_events[-1].get("stage", "")
        if latest_stage.endswith(".ready"):
            startup_state = "ready"
        elif latest_stage.endswith(".timeout"):
            startup_state = "timeout"
        elif latest_stage.endswith(".error"):
            startup_state = "error"
    return {
        "startup": {
            **startup_snapshot,
        ...
        "replay_visibility": {
            ...
            "status": "ready" if REPLAYS_DIR.exists() else "missing",
        },
...
class LiveMapRequestHandler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if self.path.startswith("/api/replays/"):
                ...
                if len(parts) > 1:
                    ...
                    if sub_route == "alignment":
                        self._json(replay_routes.get_alignment_lookup(session_id)); return
                    if sub_route == "media":
                        media_path = replay_routes.get_replay_media_path(session_id)
                        self._raw(200, "audio/wav", media_path.read_bytes()); return
    ...
    def _handle_export(self) -> None:
        ...
        fmt = parts[0]
        if fmt not in {"json", "mermaid", "markdown"}:
            self._json_error(400, f"Unsupported export format: {fmt}"); return
        target = unquote(parts[1]) if len(parts) > 1 else ""
        if target:
            data = map_store.load_map(target)
            if not data:
                self._json_error(404, f"Map target {target} not found"); return
            graph = (data or {}).get("graph", {})
        else:
            graph = STATE.graph or (STATE.session.mapper.graph() if STATE.session else {})
        if not graph:
            self._json_error(400, "No graph available for export"); return
        if fmt == "json":
