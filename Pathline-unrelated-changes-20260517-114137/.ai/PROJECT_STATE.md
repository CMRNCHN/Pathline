...
- **API**: `/api/replays` (index), `/api/replays/<id>` (state), `/api/replays/<id>/events` (raw), plus `/timeline`, `/state/<offset>`, `/cursor/<offset>`, `/diff/<from>/<to>`, `/waveform`, and `/alignment`.
- **Media Playback**: `/api/replays/<id>/media` serves locally resolved WAV recordings when present; replay cursor movement seeks the review audio player to deterministic media offsets.
- **Waveform Metadata**: Local WAV recordings generate bounded deterministic peak/RMS buckets on demand; missing or unreadable recordings return explicit structured fallback states.
- **Frontend Hydration**: `replay.js` owns staged replay loading and hydrates the review workspace without live websockets.
